...
switch model
    ...
    case 'branching_Rag_fv'
        ...
        parameters.min = [-10.3616*ones(1,12),-11.5129*ones(1,12),-6*ones(1,12),-10.3616,-10.3616];
        parameters.max = [0*ones(1,12),0*ones(1,12),5*ones(1,12),4.6052,4.6052];
        modelfun = @ simulate_pd_branching_Rag_fv;
end

options.grid = x;

% artificially shift cell at s=0 to s=e-14
D.ind.b1.hist{3}(151) = 10^(-20);
...
options.x_combined{2,k1} = union(options.grid(22:end-1)',D.b2.xsdt{it});
...
for it = 2:length(D.pop.t)
    ...
    for ig = 1:length(options.grid(22:end-1))
        e_i = zeros(1,length(options.grid(22:end-1)));
        e_i(ig) = 1;
        options.Aug_matrix{2,it}(:,ig) = augment_cdf(options.grid(22:end-1)',options.x_combined{2,it},e_i');
    end
end

                              
% Log-likelihood function
objectiveFunction = @(theta) llPseudodynamicsFvMaehrRagKSb(theta,modelfun,D,options);
...
optionsMultistart.start_index = 80 + mod(n-1,40)+1;
...
save(['parametersRag_' num2str(n)],'parameters','ic','options','optionsMultistart')
end
