function [] = mQ_st_logN_fun(n)
% compute the n-th start of a multistart optimization of the parameters
% for the model with state- and time-dependent growth rate for the Qiu data
% using PESTO
...
switch model
    ...
    case 'finiteVolume_st'
            parameters.name = {'D1','D2','D3','D4','D5','D6','D7','D8','D9',...
                'v1','v2','v3','v4','v5','v6','v7','v8','v9',...
                'a1','a2','a3','a4','a5','a6','a7','a8','a9',...
                'at1','at2','at3'};
            parameters.number = length(parameters.name);
            parameters.min = [-10.3616*ones(1,9),-11.5129*ones(1,9),-3*ones(1,12)];
            parameters.max = [0*ones(1,9),0*ones(1,9),1*ones(1,12)];
            modelfun = @ simulate_pd_fv_st;       
    case 'branching_fv'
...
objectiveFunction = @(theta) lsPseudodynamicsQiuLogNst(theta,modelfun,D,options);
...
if n==1
    save(['parametersQiu_st_logN_' num2str(n)],'parameters','options','optionsMultistart')
elseif n==101
    save(['parametersQiu_st_logN_' num2str(n)],'parameters','options','optionsMultistart')
elseif n==201
    save(['parametersQiu_st_logN_' num2str(n)],'parameters','options','optionsMultistart')
...
elseif n==501
    ...
    save(['parametersQiu_st_logN_' num2str(n)],'parameters')
end
