function [] = mQ_t_logN_fun(n)
% compute the n-th start of a multistart optimization of the parameters
% for the model with time-dependent growth rate for the Qiu data using PESTO
...
switch model
    case 'finiteVolume'
            ...
            parameters.max = [0*ones(1,9),0*ones(1,9),3*ones(1,9)];
...
if n==1
    save(['parametersQiu_t_logN_' num2str(n)],'parameters','options','optionsMultistart')
elseif n==101
    save(['parametersQiu_t_logN_' num2str(n)],'parameters','options','optionsMultistart')
elseif n==201
    save(['parametersQiu_t_logN_' num2str(n)],'parameters','options','optionsMultistart')
...
elseif n==501
    ...
    save(['parametersQiu_t_logN_' num2str(n)],'parameters')
end
