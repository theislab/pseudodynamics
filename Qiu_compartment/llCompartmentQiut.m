function [negLogL,grad] = llCompartmentQiut(varargin)
...
% regularization
negLogL = negLogL + options.alpha*(sum((theta(4:5)-theta(3:4)).^2)+...
    sum((theta(7:8)-theta(6:7)).^2));
grad = grad + options.alpha*2*([0;0;-(theta(4:5)-theta(3:4));0;...
    -(theta(7:8)-theta(6:7));0]...
    +[0;0;0;theta(4:5)-theta(3:4);0;theta(7:8)-theta(6:7)]);
end