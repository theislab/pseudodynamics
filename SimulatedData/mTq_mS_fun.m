function [] = mTq_mS_fun(n)
% compute the n-th start of a multistart optimization of the parameters
% for the simulated quasi steady state data set using PESTO
n = str2num(n);
...
switch model
    case 'finiteVolume'
            ...
            parD = -7;
            parV = -2;
            parameters.guess = [parD*ones(9,1);parV*ones(9,1);1;1;1;1.5;3.2;-0.8;-5;-6;-7];
...
if n==1
    save(['parametersQuasiSteady_' num2str(n)],'parameters','options','optionsMultistart')
elseif n==81
    save(['parametersQuasiSteady_' num2str(n)],'parameters','options','optionsMultistart')
elseif n==161
    save(['parametersQuasiSteady_' num2str(n)],'parameters','options','optionsMultistart')
else
    save(['parametersQuasiSteady_' num2str(n)],'parameters')
end
end