function [LogL,grad] = llPseudodynamicsFvKS_wBcorr(varargin)
% compute log-likelihood for pseudodynamics with bias correction for linear
% bias
...
pb = bsxfun(@rdivide,u,N)*(1/(n_grid-1));
dpbdtheta = 1/(n_grid-1)*bsxfun(@rdivide,bsxfun(@times,N,dudtheta)-...
    bsxfun(@times,u,dNdtheta),N.^2);

% compute biased probability
grid_x = linspace(options.grid(1)+(options.grid(2)-options.grid(1))/2,...
    options.grid(end-1)+(options.grid(end)-options.grid(end-1))/2,n_grid-1);
bias = options.a*grid_x+options.b;
pb = bsxfun(@times,pb,bias);
dpbdtheta = bsxfun(@times,dpbdtheta,bias);
% normalization
sp = sum(pb,2);
dspdtheta = sum(dpbdtheta,2);
p = bsxfun(@rdivide,pb,sp);
dpdtheta = bsxfun(@rdivide,bsxfun(@times,sp,dpbdtheta)-...
    bsxfun(@times,pb,dspdtheta),sp.^2);
...
for it = 2:length(D.pop.t)
    ...
    dareadtheta(it,:) = (diff(x_combined).*sign(cs_a(1:end-1)-...
        csd_a(1:end-1)))'*(dcs_adtheta(1:end-1,:));
    LogL = LogL + 0.5*log(2*pi*D.dist.var(it)) +...
        0.5*(D.dist.mean(it)-area(it)).^2/D.dist.var(it);
    grad = grad - ((D.dist.mean(it)-area(it))/D.dist.var(it)*dareadtheta(it,:))';
end

% population level statistics
for it = 2:length(D.pop.t)
    LogL = LogL + 0.5*log(2*pi*D.pop.var(it)/options.n_exp) +...
...
% regularization
LogL = LogL + options.alpha*(sum((theta(2:9)-theta(1:8)).^2)+...
