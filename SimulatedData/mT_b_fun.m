...
% for the simulated branching data set using PESTO
n = str2num(n);
...
switch model
    ...
    case 'branching_fv_toy'
         parameters.name = {'D1','D2','D3','D4','D5','D6','D7','D8','D9','D10','D11','D12'...
            'v1','v2','v3','v4','v5','v6','v7','v8','v9','v10','v11','v12'...
            'a1','a2','a3','a4','a5','a6','a7','a8','a9','a10','a11','a12','d12','d21'};
        parameters.number = length(parameters.name);
        parameters.min = [-10.3616*ones(1,12),-11.5129*ones(1,12),-6*ones(1,12),-10.3616,-10.3616];
        parameters.max = [0*ones(1,12),0*ones(1,12),3*ones(1,12),4.6052,4.6052];
...
options.x_combined{2,k1} = union(options.grid(70:end-1)',D.b2.xsdt{it});
...
for it = 2:length(D.pop.t)
    ...
    for ig = 1:length(options.grid(70:end-1))
        e_i = zeros(1,length(options.grid(70:end-1)));
        e_i(ig) = 1;
        options.Aug_matrix{2,it}(:,ig) = augment_cdf(options.grid(70:end-1)',options.x_combined{2,it},e_i');
    end
end
...
optionsMultistart.start_index = mod(n-1,120)+1;
                               
% Optimization
parameters = getMultiStarts(parameters, objectiveFunction, optionsMultistart);

if n==1
    save(['parametersBranching_' num2str(n)],'parameters','options','optionsMultistart')
elseif n==121
    save(['parametersBranching_' num2str(n)],'parameters','options','optionsMultistart')
elseif n==241
    save(['parametersBranching_' num2str(n)],'parameters','options','optionsMultistart')
else
    save(['parametersBranching_' num2str(n)],'parameters')
end
end
